...
int main(int nCmdShow) {
    ...
    hWnd = CreateWindow(WMAIN_CLASSNAME, WMAIN_CAPTION,/*WS_OVERLAPPEDWINDOW*/WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, 320, 419, HWND_DESKTOP, NULL, hInstance, 0);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    static HWND button[26], hEdit, hListBox;
    PAINTSTRUCT ps;
    HDC hdc;
    int i = 0;
    int count = 0, a = 0, b = 0;
    ...
    {
    case WM_CREATE:
        hEdit = CreateWindow(("edit"), ("0"), WS_CHILD | WS_VISIBLE | WS_BORDER | ES_RIGHT, 6, 10, 293, 115, hWnd, (HMENU)ID_EDIT, hInstance, 0);
        ...
        button[4] = CreateWindow("button", "AC", WS_VISIBLE | WS_CHILD, 185, 130, 55, 45, hWnd, (HMENU)ID_BUTTON_AC, hInstance, 0);
        button[5] = CreateWindow("button", "√", WS_VISIBLE | WS_CHILD, 125, 130, 55, 45, hWnd, (HMENU)ID_BUTTON_PR, hInstance, 0);
        ...
        button[20] = CreateWindow("button", "x", WS_VISIBLE | WS_CHILD, 65, 130, 55, 45, hWnd, (HMENU)ID_BUTTON_X, hInstance, 0);
        button[21] = CreateWindow("button", "=", WS_VISIBLE | WS_CHILD, 5, 130, 55, 45, hWnd, (HMENU)ID_BUTTON_Q, hInstance, 0);
        button[22] = CreateWindow("button", "^", WS_VISIBLE | WS_CHILD, 245, 130, 55, 45, hWnd, (HMENU)ID_BUTTON_POW, hInstance, 0);
        ...
        button[25] = CreateWindow("button", "tan", WS_VISIBLE | WS_CHILD, 245, 280, 55, 45, hWnd, (HMENU)ID_BUTTON_TAN, hInstance, 0);
        break;
    ...
    case WM_COMMAND:
        ...
        if ((LOWORD(wParam) == ID_BUTTON_X) && (HIWORD(wParam) == BN_CLICKED))
        {
            GetWindowText(hEdit, buf, sizeof(buf));
            if (*buf == '0' && 0 == buf[1])*buf = 0;
            StrCat(buf, TEXT("x"));
        ...
        if ((LOWORD(wParam) == ID_BUTTON_Q) && (HIWORD(wParam) == BN_CLICKED))
        {
            GetWindowText(hEdit, buf, sizeof(buf));
            StrCat(buf, TEXT("="));
            SetWindowText(hEdit, buf);
